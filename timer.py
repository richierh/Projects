...
class frame2(object):
    def __init__(self):
        ...
        self.button3=tk.Button(self.Frame,text="Tutup",width=10,height=2)
        self.button3.grid(row=2,column=1)
        self.button3.bind("<Button-1>",self.tutup)
    \
    def tutup(self,event):
        self.utama.destroy()
        self.k = FullScreenApp(master)
        self.k.setting()
        self.tutup()
...
class frame(object):
    ...
    def selesai(self,event):
        ...
        print("selesai dan buka tampilan utama")
        a = FullScreenApp(master)
        a.show(self)           
        

        pass
    def minimalkan(self,event):
        print ("kecilkan ukuran")
