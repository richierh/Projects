...
class Sales_Panel(wx.Panel):
	...
	#output = '<html><style>@page{ margin: 0cm;}</style><body>'
	output="""
	<html>
	<style>
	@page {
	...
	output += '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'
	output += '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'
	output += '<b>SKANDA</b><br>'
	output += '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'
	output += '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'
	output += 'CASH BILL<br>'
	output += '<b> Jayanagar Home Needs </b><br>'
	output += '#266,5th main, 4th block<br>'
	output += 'Jayanagar, Bangalore 560011<br>'
	output += '(Tel): 080-22458038<br>'
	output += '(Mob): +91-9341230258<br>'
	output += 'TIN: 29690210850</p>'
        output += '' + sale_date + ''
        #output += ' ' + (75 * '-')+'<br/>'
	...
	output +='<p style="margin-left:0;">'
        # Add Labels
        labels = 'Item', 'Price', ' * Qty', 'Amt'
        output += '<b>'+self.FormatReceiptItem(labels)+'</b>'
        #output += self.FormatReceiptItem(labels)
        #output += '<p>'+(75 * '=')+'</p>'
        
        # Add Items
        for item in receipt_list:
            output += self.FormatReceiptItem(item)
        ...
        output += '<b>'+'Sub-Total =     '+sub_total+'</b><br>'
        output += '<b>'+'Sales Tax =     '+tax+'</b><br>'
        #output += '<p><pre><b>'+(35*' ')+(40*'-')+'</b></pre></p>'
        output += '<b>'+'Total     =     '+total+'</b>'
        
	#output += '</p>'
	output += '</p>Thankyou | Visit Again</body></html>'
	print output
        #self.printer = mpos_print.Printer()
        #self.printer.PreviewText(output, '')
        self.printer = mpos_print.PrinterCups()
    ...
    def FormatReceiptItem(self, receipt_item):
        ...
        # [itm_name, price, qty, amnt]
	col_widths = 35, 25, 25, 15
        output = ''
	...
	i = 3
	if len(receipt_item[i]) < col_widths[i]:
		output += (col_widths[i]-len(receipt_item[i]))*'.' + receipt_item[i]
	else:
		output += receipt_item[i][:col_widths[i]]
	...
	i = 1
	if len(receipt_item[i]) < col_widths[i]:
		output += receipt_item[i] #+ ((col_widths[i]-len(receipt_item[i]))/2)*'.'
	else:
		output += receipt_item[i][:col_widths[i]]
